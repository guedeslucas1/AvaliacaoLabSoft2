...
from .serializers import (TimeSlotSerializer, UserSerializer, LugarEstadioSerializer    )
...
class LugaresViewSet(ViewSet):
    @swagger_auto_schema(
        method='get',
        responses={200: LugarEstadioSerializer(many=True)},
        operation_description="Retorna uma lista de lugares."
    )
    @action(detail=False, url_path='')
    def todos(self, request):
        lugar_estadio = LugarEstadio.objects.all()
        serializer = LugarEstadioSerializer(lugar_estadio, many=True)
...
@login_required
def atletas(request):
    response = requests.get("http://localhost:8001/api/lugares/disponiveis/")
    lugares = list()
    
    for slot in response.json():
        print(slot)
        name = slot["linha"] + "-" + slot["coluna"]
        id = slot["linha"] + slot["coluna"]
        lugares.append(
            {
               "name": name,
               "atl_id": id,
    ...
    return render(request, 'trainer_portal/atletas.html', {"athletes_view":athletes_view})


def perfil_atleta(request, atl_id):

    if request.method == 'POST':
        print("COMPRAR INGRESSO:", atl_id[0], atl_id[1:])
        obj = LugarEstadio.objects.get(linha=str(atl_id[0]), coluna=atl_id[1:])
        print(obj)

        obj.disponivel = False
        obj.save()
        return redirect(reverse('trainer-portal-atletas'))
